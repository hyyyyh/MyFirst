    ...
    $.fn.startProgress=function(data){
        ...
        if(data&&data.nodes&&data.hasPay){
            ...
            for(var node in data.nodes){
                if(data.nodes[node].num<=data.hasPay){
                    currentNode+=1;
                    temp=data.nodes[node].num;
                }
                overMoney=data.hasPay-temp;
                hStr='<div class="yifen dengji '+(node>0?'gray':'')+'">'
                    +'<img src="../img/xingsuo.png">'
                    +'<div class="jucol">'+data.nodes[node].name+'</div>'
                    +'<div>'+(node>0&&node==5?data.nodes[node].num/10000+"万以上":data.nodes[node].num)+'</div>'					
                    +'</div>';
                ele.find(".djwai").append(hStr);
            }
            $(".flexbox img").eq(0).attr("src","../img/xing0.png");
            ...
            if(currentNode&&currentNode>0&&currentNode<=data.nodes.length){
                if(currentNode==data.nodes.length){
                    ...
                    redLine+=(overMoney/(data.nodes[currentNode].num
                        -data.nodes[currentNode-1].num))*(1/(data.nodes.length-1));
                    redLine=redLine*100;
        ...
        /*设置进度条当前位置*/
        function setPosition(p,num){
            for(var n in data.nodes){
                if(p/100>=n/(data.nodes.length-1)){
                    $(".yifen").eq(n).removeClass('gray');
                    $(".flexbox img").eq(0).attr("src","../img/xing0.png");
                    $(".flexbox img").eq(1).attr("src","../img/xing1.png");
                    $(".flexbox img").eq(2).attr("src","../img/xing2.png");
                    $(".flexbox img").eq(3).attr("src","../img/xing3.png");
                    $(".flexbox img").eq(4).attr("src","../img/xing4.png");
                    $(".flexbox img").eq(5).attr("src","../img/xing5.png");
                    $(".gray img").attr("src","../img/xingsuo.png");
                }
            }
            ele.find(".dingwei").css("left",p+'%');
            ele.find(".hongxian").css("width",p+'%');
            ele.find(".yuandian").css("left",p+'%');
            if(p>96){
                ele.find(".yixiaof").css("left",'96%');
            }else{
                ele.find(".yixiaof").css("left",p+'%');
            }
          
            //ele.find(".yixiaof").text(num);
        }
    }})();